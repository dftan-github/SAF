...
- 由Ruby言语为主进行程序逻辑控制的RAD级通用桌面应用程序框架(2D、3D共存，类似插件式开发，可任意扩充功能。目前是在Windows平台下做测试,其它主要平台应该也是可行的,因为是基于glfw+opengl4.5+自绘方式)
...
- 基于SAF提供的基础及概念，可以预见到它能胜任N多你想要做的任务（除了速度未知，但是基于硬件回速的渲染，效率不会差，目前UI绘制在我的机子上120FPS以上（实际是150FPS上下)）
